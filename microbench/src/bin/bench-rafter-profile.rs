...
fn profile_serial(proposals: usize, payload_bytes: usize) -> ProfileReport {
    ...
    for _ in 0..proposals {
        let payload = profile.payload.observe(|| payload_of_size(payload_bytes));
        ...
        let outputs = profile.leader_step.observe(|| cluster.step_leader(input));
...
fn print_report(report: &ProfileReport) {
    ...
    print_phase("queue_extend", &report.profile.queue_extend, report.elapsed);
}
